...
//			| dynamic arrays
//			| each value of the result matrix is performed in a thread
//		
//		--> Using complex Data structure and pointers
//
//-------------------------------------------------------------------------------------------------------------------------
	...
	if (data->matrix1->column == data->matrix2->line)
	{
		for (int i = 0; i<data->matrix3->line; i++)
		{
			for (int j = 0; j<data->matrix3->column; j++)
			{
				...
				{
					value = value + data->matrix1->array[data->i][k] * data->matrix2->array[k][data->j];
				}
				data->matrix3->array[data->i][data->j] = value;
				#ifdef DEBUG_ON
					std::cout << "\ni = " << data->i << std::endl;
					std::cout << "\nj = " << data->j << std::endl;
					std::cout << "\nvalue [" << data->i << "][" << data->j << "] = " << data->matrix3->array[data->i][data->j];
				#endif // DEBUG_ON
			}
		}
