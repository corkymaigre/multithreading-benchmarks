...
//			| each row in the result matrix is performed by one thread
//
//-------------------------------------------------------------------------------------------------------------------------
...
static void *MutiplyWithThread(void *arg)
{
	...
	for (int a = 0; a < ldata->size; a++)
	{
		...
		data->matrix3->array[ldata->cells[a].i][ldata->cells[a].j] = value;
#ifdef DEBUG_ON
		std::cout << "\ni = " << ldata->cells[a].i << std::endl;
		std::cout << "\nj = " << ldata->cells[a].j << std::endl;
		std::cout << "\nvalue [" << ldata->cells[a].i << "][" << ldata->cells[a].j << "] = " << data->matrix3->array[ldata->cells[a].i][ldata->cells[a].j];
#endif // DEBUG_ON
	}
...
static void Multiply(Data *data)
{
	...
	{
		// Create threads
		for (int n = 0; n < data->matrix3->line; n++)
		{
			pthread_create(&data->thread[n], NULL, MutiplyWithThread, (void*)&data->thread_data[n]);
		}
	...
	else
	{
		...
		system("pause");
		flag->is_matrix_multiplied = false;
	}
}
...
static void AllocateThread(Data *data)
{
	data->thread = new pthread_t[data->matrix3->line];				// array of threads
	data->thread_data = new ThreadData[data->matrix3->line];		// array of data for threads

																	// foreach thread
	for (int n = 0; n < data->matrix3->line; n++)
	{
		// define the number of cells
		data->thread_data[n].size = data->matrix3->column;
		// create the dynamic array of cells
		data->thread_data[n].cells = new Cell[data->thread_data[n].size];
		// foreach cell set the i and j coordinates
		for (int a = 0; a < data->thread_data[n].size; a++)
		{
			data->thread_data[n].cells[a].i = n;
			data->thread_data[n].cells[a].j = a;
		}
	}
}
#pragma endregion
